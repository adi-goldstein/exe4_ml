...
def train(opt, device):
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        tloss, vloss, fitness, train_acc = 0.0, 0.0, 0.0, 0.0  # train loss, val loss, fitness
        model.train()
        if RANK != -1:
        ...
        for i, (images, labels) in pbar:  # progress bar
            ...
            with torch.no_grad():
                ...
                total = labels.size(0)
                batch_train_acc = correct / total

            # Update running train accuracy
            train_acc = (train_acc * i + batch_train_acc) / (i + 1)
